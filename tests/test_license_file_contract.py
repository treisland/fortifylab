...
class LicenseFileContractTests(unittest.TestCase):
    ...
    def test_helm_consumers_use_existing_secret_without_license_values(self) -> None:
        for relative_path in (
            ...
            self.assertRegex(
                script,
                r"(?:secretRef\.name|secrets\.secretName|sscLicenseSecret)=fortify-secrets",
            )
    ...
    def test_secret_regeneration_preserves_live_ssc_encryption_key(self) -> None:
        script = (ROOT / "scripts/create-secrets.sh").read_text(encoding="utf-8")
        preserve = script.index("get secret fortify-secrets")
        rebuild = script.index('rm -rf "$GENERATED_DIR"')
        delete = script.index('$KUBECTL -n "$NAMESPACE" delete secret')
        self.assertLess(preserve, rebuild)
        self.assertLess(preserve, delete)
        self.assertIn('cp "$PRESERVED_SECRET_KEY" "$SSC_GEN_DIR/secret.key"', script)
